from Models.Tree_node import Tree_node
import hashlib
...
class Tree:
    ...
    def get_artical(self,point,list):    #输入默克尔树的根，以及一个空列表用于储存提取的文章信息
        if(point!=None):
            if(point.lchild==None and point.rchild==None):
                list.append(point.data)
            else:
                self.get_artical(point.lchild,list)
                self.get_artical(point.rchild,list)
if __name__ == "__main__":
    tree=Tree()
    list=['a',"b","c","d","e","f"]              #模拟原始数据
    artical_list=['a',"b","f","d","e","f"]    #模拟数据被篡改
